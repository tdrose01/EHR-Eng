...
Write-Section "Fixing Vaccine Test File"

# First make a backup of the original file
$testFile = "tests/vaccine-test.js"
$backupFile = "tests/vaccine-test.js.bak"

if (Test-Path $testFile) {
    Copy-Item $testFile $backupFile -Force
    Write-Host "Created backup at $backupFile" -ForegroundColor Green
} else {
    Write-Host "Vaccine test file not found at $testFile!" -ForegroundColor Red
    exit 1
}
...
# Make sure file has the right format and structure
$updatedContent = @"
// @ts-check
...
test.describe('Vaccine functionality', () => {
  ...
  test('should load vaccine API and display vaccines', async ({ page }) => {
    ...
    for (let i = 1; i < options.length; i++) { // Skip the first placeholder
      ...
      console.log(`Vaccine option: \${text} (value: \${value})`);
    }
  });
  ...
  test('should save vaccination record properly', async ({ page }) => {
    ...
    if (options.length > 1) {
      ...
      const testNotes = `Automated test vaccination record \${timestamp}`;
      ...
      const recordText = page.locator(`table tbody tr:has-text("\${testNotes}")`);
...
$updatedContent | Set-Content $testFile -Force
Write-Host "Updated vaccine test file with proper structure" -ForegroundColor Green

# Also create a simpler alternative version if needed
$alternativeTest = @"
// @ts-check
const { test, expect } = require('@playwright/test');
...
Set-Content -Path "tests/simple-vaccine-test.js" -Value $alternativeTest
Write-Host "Created simplified vaccine test at tests/simple-vaccine-test.js" -ForegroundColor Green

Write-Section "Running Tests"
Write-Host "Now try running the tests with one of these commands:" -ForegroundColor Green
Write-Host "  1. Regular test:  .\run_vaccine_test.ps1" -ForegroundColor White
Write-Host "  2. Simple test:   npx playwright test tests/simple-vaccine-test.js --headed" -ForegroundColor White
Write-Host "  3. Batch file:    .\vaccine_test_direct.bat" -ForegroundColor White 